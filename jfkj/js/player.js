...
APlayer.prototype.init = function () {
    ...
    this.element.innerHTML = ''
        ...
        +         '<span class="aplayer-author">' + this.music.author + '</span>'
        +     '</div>'
        ...
        +             ' - <span class="aplayer-ptime">00:00</span> / <span class="aplayer-dtime">(o锞熲柦锞�)</span>'
    ...
    this.audio.addEventListener('error', function () {
        _self.element.getElementsByClassName('aplayer-author')[0].innerHTML = ' - ' + '鍔犺浇澶辫触 鈺ワ箯鈺�';
    });
    ...
    if (currentTime < this.lrcTime[this.lrcIndex] || currentTime >= this.lrcTime[this.lrcIndex + 1]) {
        for (var i = 0; i < this.lrcTime.length; i++) {
            if (currentTime >= this.lrcTime[i] && (!this.lrcTime[i + 1] || currentTime < this.lrcTime[i + 1])) {
                this.lrcIndex = i;
                this.lrcContents.style.transform = 'translateY(' + -this.lrcIndex * 27 + 'px)';
